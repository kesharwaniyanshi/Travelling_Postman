...
class MultiModalRouteOptimizer {
  ...
  // Dijkstra's algorithm optimized for cost and infrastructure
    ...
    for (let count = 0; count < n; count++) {
      ...
      for (let next = 0; next < n; next++) {
        ...
        const validRoutes = transportModes
          .map(mode => {
            // Extract cost and check infrastructure
            const sourceCity = this.cities[current];
            const destCity = this.cities[next];
            ...
            return {
              mode,
              cost: routeExists && cost < this.MAX_COST_THRESHOLD ? cost : Infinity,
              time: routeExists ? time : Infinity
            };
          })
          .filter(route => route.cost !== Infinity);

        if (validRoutes.length === 0) continue;

        // Optimize for cost instead of time
  ...
  // Yen's K-Shortest Paths Algorithm with cost optimization
  findTopMultiModalRoutes(source, destination, topK = 5) {
    ...
    if (shortestPathDetails.totalCost !== Infinity) {
      shortestPaths.push({
    ...
    for (let k = 1; k < topK; k++) {
      ...
      for (let i = 0; i < shortestPaths[k-1].path.length - 1; i++) {
        ...
        if (spurPath) {
          ...
          // Only add valid paths with reasonable cost
          if (pathDetails.totalCost !== Infinity) {
            candidatePaths.push({
              path: fullPath,
              pathModes: fullModes,
      ...
      // Sort and select next shortest path based on cost
